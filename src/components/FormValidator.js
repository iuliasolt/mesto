export class FormValidator {
  ...
  _checkInputValidity(inputElement) {
      if (!inputElement.validity.valid) {
          //Проверка валидации значений содержащ. в поле ввода inputElement
  ...
  _hasInvalidInput() {
      return this._inputList.some((inputElement) => {
          //Для перебора значений массива используем функцию some(). Если одно из полей неккоретно, то функция вернет истину, иначе ложь.
  ...
  toggleButtonValidity() {
      if (this._hasInvalidInput()) {
          //Вызов функции hasInvalidInput с передачей массива полей ввода.
